...
class LitModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        weights = self.attention(last_layer_hidden_states)  # [16, 248, 1]
        ...
        context_vector = torch.sum(
            weights * last_layer_hidden_states, dim=1)  # [16, 768]

        # Now we reduce the context vector to the prediction score.
        return self.regressor(context_vector)  # [16, 1]
        ...
        for batch_num, (input_ids, attention_mask, target) in enumerate(train_loader):
            ...
            if step >= last_eval_step + eval_period:   # 最初は16イテレートごとに評価
                ...
                with open(base_path + '/log.txt', 'a') as flog:
                    flog.write(
                        f"Epoch: {epoch} batch_num: {batch_num} val_rmse: {val_rmse:0.4}\n")
